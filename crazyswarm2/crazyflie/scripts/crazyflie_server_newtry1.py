...
# from crazyflie_interfaces.msg import State ########
...
class CrazyflieServer(Node):
    def __init__(self):
        ...
        ##################################################################################
        for link_uri in self.uris:
            self.swarm._cfs[link_uri].cf.power_switch = PowerSwitch(link_uri)
        ########################### CREATING NEW SERVICES ################################
        self.create_service(Trigger, 'power_down', self.handle_power_down)
        self.create_service(Trigger, 'power_up', self.handle_power_up)
        ##################################################################################
...
############################### NEW STUFF ###############################
    def power_down(self):
        try:
            # Ensure that the Crazyflie is not in the middle of a command
            self.crazyflie.commander.send_stop_setpoint()
            # Power down the Crazyflie
            self.power_switch.stm_power_down()
        except Exception as e:
            self.get_logger().error(f"Failed to power down: {e}")
    
    def power_up(self):
        try:
            # Ensure that the Crazyflie is not in the middle of a command
            self.crazyflie.commander.send_stop_setpoint()
            # Power up the Crazyflie
            self.power_switch.stm_power_up()
        except Exception as e:
            self.get_logger().error(f"Failed to power up: {e}")

    # def handle_power_down(self, request, response, uri="all"):
    #     """
    #     Service callback to power down the Crazyflie(s)
    #     """
    #     self.get_logger().info(f"Powering down Crazyflie(s) with URI: {uri}")
        
    #     try:
    #         if uri == "all":
    #             for link_uri in self.uris:
    #                 self.swarm._cfs[link_uri].cf.commander.send_stop_setpoint()
    #                 self.swarm._cfs[link_uri].cf.power_switch.stm_power_down()
    #         else:
    #             self.swarm._cfs[uri].cf.commander.send_stop_setpoint()
    #             self.swarm._cfs[uri].cf.power_switch.stm_power_down()

    #         self.get_logger().info(f"Successfully powered down Crazyflie(s) with URI: {uri}")
    #     except Exception as e:
    #         self.get_logger().error(f"Failed to power down: {e}")
    #         response.success = False
    ...
    #     response.success = True
    #     response.message = f"Powered down Crazyflie(s) with URI: {uri}"
    #     return response
    
    # def handle_power_up(self, request, response, uri="all"):
    #     """
    #     Service callback to power up the Crazyflie(s)
    #     """
    #     self.get_logger().info(f"Powering up Crazyflie(s) with URI: {uri}")
        
    #     try:
    #         if uri == "all":
    #             for link_uri in self.uris:
    #                 self.swarm._cfs[link_uri].cf.commander.send_stop_setpoint()
    #                 self.swarm._cfs[link_uri].cf.power_switch.stm_power_up()
    #         else:
    #             self.swarm._cfs[uri].cf.commander.send_stop_setpoint()
    #             self.swarm._cfs[uri].cf.power_switch.stm_power_up()

    #         self.get_logger().info(f"Successfully powered up Crazyflie(s) with URI: {uri}")
    #     except Exception as e:
    #         self.get_logger().error(f"Failed to power up: {e}")
    #         response.success = False
    #         response.message = str(e)
    ...
    #     response.success = True
    #     response.message = f"Powered up Crazyflie(s) with URI: {uri}"
    #     return response
    ...
    def handle_power_down(self, request, response, uri="all"):
        ...
        try:
            if uri == "all":
                for link_uri in self.uris:
                    self.swarm._cfs[link_uri].cf.commander.send_stop_setpoint()
                    # Power down the STM MCU using PowerSwitch
                    self.swarm._cfs[link_uri].cf.power_switch.stm_power_down()
            else:
                self.swarm._cfs[uri].cf.commander.send_stop_setpoint()
                # Power down the STM MCU using PowerSwitch
    ...
    def handle_power_up(self, request, response, uri="all"):
        ...
        try:
            if uri == "all":
                for link_uri in self.uris:
                    self.swarm._cfs[link_uri].cf.commander.send_stop_setpoint()
                    # Power up the STM MCU using PowerSwitch
                    self.swarm._cfs[link_uri].cf.power_switch.stm_power_up()
            else:
                self.swarm._cfs[uri].cf.commander.send_stop_setpoint()
                # Power up the STM MCU using PowerSwitch
